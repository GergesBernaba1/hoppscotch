      ...
      <div class="sticky z-10 flex flex-col bg-primary top-upperRequestRect">
        ...
        <div v-if="activeTab === 'wsdl'" class="flex flex-col flex-1 p-4">
          ...
          <div class="flex flex-col pb-4 border-b">
            <label class="pb-1 font-semibold">{{ t('soap.wsdl_source') }}</label>
            ...
            <div v-if="wsdlSourceType === 'file'">
              <label class="pb-1 font-semibold">{{ t('soap.wsdl_file') }}</label>
        ...
        <div v-else-if="activeTab === 'operation'" class="flex flex-col flex-1 p-4">
        ...
        <div v-else-if="activeTab === 'params'" class="flex flex-col flex-1 p-4">
          <!-- Params editor component -->
          <ParametersComponent v-model="params" />
        </div>
          <div v-else-if="activeTab === 'headers'" class="flex flex-col flex-1 p-4">
          ...
          <div v-else-if="activeTab === 'attachments'" class="flex flex-col flex-1 p-4">
          <div class="flex items-center justify-between pb-4">
    ...
    >
      <template #body>
        <div class="flex flex-col space-y-4">
          <p class="text-secondary">{{ t('soap.wsdl_import_options_description') }}</p>
          
          <!-- Project name input -->
          <div class="flex flex-col space-y-1">
            <label for="projectName" class="text-secondary font-semibold">{{ t('soap.project_name') }}</label>
            <input
              id="projectName"
          ...
          <div class="flex flex-col space-y-2">
            <label class="flex items-center space-x-2">
              ...
              />
              <span class="text-secondary">{{ t('soap.create_sample_requests') }}</span>
            </label>
            ...
            <label class="flex items-center space-x-2">
              <input
                ...
                class="h-4 w-4 text-accent border-divider rounded focus:ring-accent"
              />
              <span class="text-secondary">{{ t('soap.create_test_suite') }}</span>
            </label>
            ...
            <label class="flex items-center space-x-2">
              <input
                ...
                class="h-4 w-4 text-accent border-divider rounded focus:ring-accent"
              />
              <span class="text-secondary">{{ t('soap.create_simulation') }}</span>
            </label>
          </div>
        </div>
      </template>
...
import { HoppButtonPrimary, HoppButtonSecondary, HoppSmartTabs, HoppSmartTab, HoppSmartModal } from "@hoppscotch/ui"
...
const currentParsedWSDL = ref<{ operations: WSDLOperation[], services: WSDLService[], targetNamespace: string } | null>(null)
...
const fetchWSDLDetails = async () => {
  ...
  try {
    ...
    currentParsedWSDL.value = { operations, services, targetNamespace: targetNamespace || "" }
    ...
    try {
      ...
      const projectName = lastPart.replace(/\.(wsdl|xml)$/i, '') || 'soap-project'
      wsdlImportOptions.value.projectName = projectName
    }
...
const sendRequest = async () => {
  ...
  try {
    ...
    if (Array.isArray(endpointSuggestions.value) && !endpointSuggestions.value.includes(endpoint.value)) {
...
const applyWSDLImportOptions = () => {
  ...
  if (hasOperationsWithAction && !headers.value.find(h => h.key.toLowerCase() === 'soapaction')) {
    headers.value.push({
      key: 'SOAPAction',
  ...
  if (wsdlImportOptions.value.createSampleRequests) {
    ...
    if (operations.length > 0) {
      ...
      if (firstOperation.soapAction) {
        const soapActionHeader = headers.value.find(h => h.key.toLowerCase() === 'soapaction')
        if (soapActionHeader) {
      ...
      console.log(`Generated sample requests for ${operations.length} operations:`)
      operations.forEach(op => {
        console.log(`- ${op.name}${op.soapAction ? ` (Action: ${op.soapAction})` : ''}`)
      })
  ...
  if (wsdlImportOptions.value.createSimulation) {
    ...
    const simulationScript = generateSimulationScript(operations, targetNamespace)
    ...
    console.log("SOAP simulation mode enabled - requests will be intercepted by pre-request script")
  }
...
const generateSampleSoapEnvelope = (operation: WSDLOperation, namespace: string): string => {
  const soapNs = soapVersion.value === SOAP_VERSION_1_1 
...
const generateSimulationScript = (operations: WSDLOperation[], namespace: string): string => {
...
switch(operation) {
${operations.map(op => `  case "${op.name}":
