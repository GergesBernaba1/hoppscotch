...
async function convertAttachments(
  ...
  return Promise.all(
    attachments.map(async (att) => ({
      ...att,
      content:
        att.content instanceof File
          ? await fileToArrayBuffer(att.content)
          : att.content,
    }))
  )
}
...
function validateSOAPEnvelope(
  ...
  try {
    ...
    if (typeof body !== "string") {
      console.error(
        "SOAP envelope validation failed: Body is not a string:",
        body
      )
      return false
    }
    ...
    if (!envelope) {
      console.error(
        "SOAP envelope validation failed: No Envelope element found"
      )
      return false
    }
    ...
    if (namespaces.length === 0) {
      console.error(
        "SOAP envelope validation failed: No namespace attributes found"
      )
      return false
    }
    ...
    if (soapVersion === "1.1") {
      ...
      if (!hasValidNS) {
        console.error(
          "SOAP 1.1 envelope validation failed: Invalid namespace",
      ...
      if (!hasValidNS) {
        console.error(
          "SOAP 1.2 envelope validation failed: Invalid namespace",
          { foundNamespaces: namespaces }
        )
      }
      return hasValidNS
    ...
    console.error(
      "SOAP envelope validation failed: Invalid SOAP version specified",
      { version: soapVersion }
    )
    return false
  } catch (error) {
    ...
    console.error(
      "SOAP request failed with error:",
...
function checkForSOAPFault(responseBody: string): {
  isFault: boolean
  faultCode?: string
  faultString?: string
} {
...
function processSOAPResponse(
  ...
  if (res.type !== "success") {
    const errorResponse: HoppSOAPResponse = {
      ...
      req,
      error: new Error(
        res.type === "fail" ? res.error.message : "Network failure"
      ),
  ...
  if (res.body && typeof res.body === "string") {
    ...
    if (faultCheck.isFault) {
      ...
      console.error(
        "SOAP request failed with error:",
        `SOAP Fault: ${faultCheck.faultCode || "Unknown"} - ${faultCheck.faultString || "Unknown error"}`
      )
  ...
  if (typeof processedBody === "string") {
    ...
    const extractOperationResult = (
      ...
      const responseRegex = new RegExp(
        `<(?:\\w+:)?${operation}Response[^>]*>([\\s\\S]*?)</(?:\\w+:)?${operation}Response>`,
        "i"
      )
      const resultRegex = new RegExp(
        `<(?:\\w+:)?${operation}Result[^>]*>([\\s\\S]*?)</(?:\\w+:)?${operation}Result>`,
        "i"
      )
    ...
    try {
      ...
      if (isUsingProxy) {
        ...
        // If we can't find a SOAP envelope but have XML, it might still be valid
          ...
          console.log(
            "Found XML in proxy response, assuming it's a valid SOAP response"
          )

          // Look for operation results without envelope
          if (req.operation) {
            const extractedResult = extractOperationResult(
              processedBody,
              req.operation
            )
            if (extractedResult) {
              console.log(
                `Found result for operation ${req.operation} without envelope`
              )
            }
          }
        }
        // Handle JSON response from certain proxies that wrap the content
          ...
          processedBody.includes('"contents"') ||
          processedBody.includes('"data"') ||
          processedBody.includes('{"status":') ||
          processedBody.includes("{")
        ) {
          console.log(
            "Processing proxy JSON response content:",
            processedBody.substring(0, 200)
          )

          // Clean and parse the JSON
          // Remove any BOM or non-printable characters
          const cleanedBody = processedBody
            .replace(/^\s+|\s+$/g, "")
            .replace(/^\ufeff/g, "")
          ...
          try {
            ...
            const xmlMatch = processedBody.match(
              /<(?:\w+:)?Envelope[^>]*>[\s\S]*<\/(?:\w+:)?Envelope>/i
            )
            if (xmlMatch && xmlMatch[0]) {
              processedBody = xmlMatch[0]
              console.log(
                "Extracted SOAP envelope using regex after JSON parse failure"
              )
            }
          ...
          if (jsonResponse && typeof jsonResponse === "object") {
            ...
            if (extractedContent) {
              ...
              if (soapEnvelope) {
                ...
                console.log(
                  "Using extracted content from proxy (no SOAP envelope found)"
                )
              }
            }
          }
      ...
      console.log(
        "Continuing with original response due to error in proxy processing"
      )
    }
  }
  ...
  ;(async () => {
    try {
      ...
      if (req.endpoint && req.operation) {
        console.log(
          `Checking if SOAPAction header is needed for operation: ${req.operation}`
        )
        // Check if there's already a SOAPAction header
        const hasSoapAction = headers.some(
          (h) => h && h.key && h.key.toLowerCase() === "soapaction" && h.active
        )
        
        if (!hasSoapAction) {
          // No need for hard-coded values - the header will be added below in the SOAP version sections
          console.log(
            "No SOAPAction header present, will be added based on SOAP version"
          )
        }
      }
      ...
      if (req.useMtom && req.attachments && req.attachments.length > 0) {
        ...
        if (req.operation) {
          ...
          console.log(
            `Added SOAPAction header for operation ${req.operation}:`,
            `"${soapActionValue}"`
          )
        }
      } else {
      ...
      if (req?.endpoint && typeof req.endpoint === "string") {
        if (
          req.endpoint.includes("dneonline.com") ||
          (!req.endpoint.includes("localhost") &&
            !req.endpoint.startsWith(window.location.origin))
        ) {
          console.warn(
            "Potential CORS issue detected with endpoint:",
            req.endpoint
          )
          ...
          if (req.endpoint.includes("dneonline.com/calculator")) {
            ...
            if (
              ...
              typeof req.preRequestScript !== "string" ||
              !req.preRequestScript.includes("PROXY_MODE")
            ) {
              ...
              try {
                req.endpoint =
                  proxyOptions[0] + encodeURIComponent(req.endpoint)
                console.log(
                  "Using corsproxy.io as the default proxy for Calculator service"
                )
              } catch (encodeError) {
                ...
                try {
                  req.endpoint =
                    proxyOptions[1] + encodeURIComponent(req.endpoint)
                } catch (error) {
              ...
              console.log(
                `Using proxy for Calculator service: Original endpoint "${originalEndpoint}" changed to "${req.endpoint}"`
              )
            }
          }
        }
      ...
      if (
        ...
        console.log(
          "Detected Calculator service with allorigins proxy - using special handling"
        )
        ...
        try {
          ...
          const originalUrl = decodeURIComponent(
            req.endpoint.replace("https://api.allorigins.win/raw?url=", "")
          )
          req.endpoint =
            "https://corsproxy.io/?" + encodeURIComponent(originalUrl)
          console.log(
            "Switched proxy from allorigins to corsproxy.io for Calculator service"
          )
        } catch (proxyError) {
      ...
      const restRequest = {
        ...
        endpoint: req.endpoint,
        auth: req.auth || null,
        headers: Array.isArray(safeHeaders)
          ? safeHeaders.filter(
              (h) => h && typeof h === "object" && "active" in h && h.active
            )
          : [],
        body: bodyContent,
        params: Array.isArray(safeParams)
          ? safeParams.filter(
              (p) => p && typeof p === "object" && "active" in p && p.active
            )
          : [],
      ...
      result.response
        .then((res: E.Either<KernelInterceptorError, RelayResponse>) => {
          ...
          if (res._tag === "Right") {
            ...
            RESTResponse.toResponse(res.right, restRequest as any)
              .then((processedRes: HoppRESTResponse) => {
                ...
                console.log("SOAP response processed:", {
                  ...
                  bodyLength:
                    typeof processedRes.body === "string"
                      ? processedRes.body.length
                      : "binary data",
                  bodyPreview:
                    typeof processedRes.body === "string"
                      ? processedRes.body.substring(0, 100)
                      : "binary data",
                })
                ...
                if (
                  processedRes.statusCode === 0 ||
                  (typeof processedRes.body === "string" &&
                    (processedRes.body.includes(
                  ...
                  const safeConvertHeaders = (headers: any[] = []) => {
                    if (!Array.isArray(headers)) {
                      console.warn(
                        "Headers is not an array in safeConvertHeaders:",
                        headers
                      )
                      return []
                    }
                  ...
                  return {
                    ...
                    meta: { corsDetected: boolean; corsMessage: string }
                  }
                }
                ...
                if (
                  processedRes.statusCode > 0 &&
                  (!processedRes.body || processedRes.body === "")
                ) {
                  console.warn(
                    "Response status indicates success but body is empty"
                  )

                  // Create a warning message
                  const warningMessage =
                    "Response status indicates success but no body was received. " +
                ...
                if (
                  processedRes.statusCode > 0 &&
                  typeof processedRes.body === "string" &&
                  !processedRes.body.trim().startsWith("<") &&
                  !processedRes.body.includes("Envelope")
                ) {
                ...
                console.log("Final SOAP response ready to send to UI:", {
                  ...
                  bodyLength: soapResponse.body
                    ? typeof soapResponse.body === "string"
                      ? soapResponse.body.length
                      : "binary"
                    : 0,
                })
              ...
              .catch((error: unknown) => {
                ...
                if (
                  ...
                  friendlyErrorMsg =
                    "Network error: Could not connect to the SOAP service. " +
                ...
                const errorResponse: HoppSOAPResponse = {
                  ...
                  error:
                    enhancedError instanceof Error
                      ? enhancedError
                      : new Error(String(enhancedError)),
            ...
            if (errorValue === "cancellation") {
              ...
              if (
                ...
                if (req.endpoint.includes("dneonline.com/calculator")) {
                  enhancedMessage =
                    "CORS policy error with the Calculator service: " +
                  ...
                  enhancedMessage =
                    "CORS policy error: The SOAP service doesn't allow requests from your browser. " +
