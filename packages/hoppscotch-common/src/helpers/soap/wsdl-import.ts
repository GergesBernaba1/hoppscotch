...
export const safeParseWSDL = (wsdlContent: string): E.Either<Error, any> => {
  try {
    ...
    const parserOptions = {
      ...
      isArray: (name: string) => ["operation", "service", "port"].includes(name),
    }
    ...
    return E.left(new Error(`Failed to parse WSDL: ${error instanceof Error ? error.message : String(error)}`))
  }
}
 ...
 */
export const extractWSDLElements = (parsedWSDL: any): { 
  ...
  if (parsedWSDL?.definitions?.portType) {
    ...
    portTypes.forEach((portType: any) => {
      if (portType.operation) {
        const ops = Array.isArray(portType.operation) ? portType.operation : [portType.operation]
  ...
  if (parsedWSDL?.definitions?.binding) {
    ...
    bindings.forEach((binding: any) => {
      if (binding.operation) {
        const ops = Array.isArray(binding.operation) ? binding.operation : [binding.operation]
        
        ops.forEach((op: any) => {
          const existingOpIndex = result.operations.findIndex(o => o.name === op["@_name"])
          if (existingOpIndex >= 0) {
            // Check for SOAP 1.1 and 1.2 operations
            const soapAction = op?.["soap:operation"]?.["@_soapAction"] || 
