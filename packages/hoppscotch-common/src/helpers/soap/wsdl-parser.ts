...
const mergeDefinitions = (mainDefs: any, importedDefs: any) => {
  ...
  for (const key of keysToMerge) {
    ...
    if (importedElement) {
      const mainItems = mainElement
        ? Array.isArray(mainElement)
          ? mainElement
          : [mainElement]
        : []
      const importedItems = Array.isArray(importedElement)
        ? importedElement
        : [importedElement]
      ...
      const existingKey = Object.keys(mainDefs).find(
        (k) => k.endsWith(`:${key}`) || k === key
      )
      if (existingKey) {
...
export const parseWSDL = async (
  ...
  try {
      ...
      const hasDefinitionsCharacteristics = (o: any) => {
        return (
          (o["wsdl:portType"] ||
            ...
            o["types"] ||
            o["wsdl:types"]) &&
          (typeof o["@_targetNamespace"] === "string" ||
            typeof o["@_xmlns"] === "string")
        )
      }
    ...
    const definitions = findDefinitionsElement(parsed)

    if (!definitions) {
      console.error("Parser Error: Could not find root <definitions> element.")
      return E.left(
        new Error("Could not find root <definitions> element in WSDL.")
      )
    }

    console.log(
      "2. Found <definitions> element (pre-import):",
    ...
    if (imports && baseUrl) {
        ...
        if (location) {
          try {
            ...
            if (E.isLeft(importedContentEither)) {
              console.warn(
                `Skipping import: Could not fetch ${importUrl}: ${importedContentEither.left.message}`
              )
              continue
            }
            ...
            console.warn(
              `Skipping import: Failed to process import at ${location}`,
              e
            )
          }
        }
      ...
      console.log(
        "2.1. Definitions after merging imports:",
        JSON.stringify(definitions)
      )
    } else if (imports && !baseUrl) {
      console.warn(
        "WSDL contains imports, but no base URL was provided. Imports will be ignored. Please use the URL option for WSDLs with imports."
      )
    }
    ...
    if (portTypeBlocks) {
      const allPortTypes = Array.isArray(portTypeBlocks)
        ? portTypeBlocks
        : [portTypeBlocks]
        ...
        if (part) {
          ...
          if (parts.length > 0) {
            ...
            if (elementPart) {
              ...
              console.log(
                `Added message ${msg["@_name"]} to map with element: ${elementPart["@_element"]}`
              )
              ...
              console.log(
                `Added message ${msg["@_name"]} to map with type: ${parts[0]["@_type"]}`
              )
            } else {
              messageMap.set(msg["@_name"], {
                name: parts[0]["@_name"],
              ...
              console.log(
                `Added message ${msg["@_name"]} to map with name only: ${parts[0]["@_name"]}`
              )
            }
          }
        }
    ...
    if (typesBlocks) {
      ...
      if (schemaNodes) {
        ...
        const schemaArray = Array.isArray(schemaNodes)
          ? schemaNodes
          : [schemaNodes]
        ...
        schemaArray.forEach((schema, schemaIndex) => {
          ...
          if (complexTypes) {
            const ctArray = Array.isArray(complexTypes)
              ? complexTypes
              : [complexTypes]
            ...
            ctArray.forEach((ct) => {
              ...
              if (ctName) {
                ...
                console.log(
                  `Added complex type schema: ${ctName} with ${fields.length} fields`
                )
              }
            })
          }
          ...
          if (elements) {
            ...
            elems.forEach((el: any) => {
              ...
              if (complexType) {
                ...
                console.log(
                  `Added element schema: ${elName} with ${fields.length} fields`
                )
              } else if (el["@_type"]) {
                ...
                console.log(
                  `Added simple element schema: ${elName} with type ${el["@_type"]}`
                )
              }
            })
          }
    ...
    if (!bindings && portTypeMap.size > 0) {
      console.log(
        "No bindings found, extracting operations directly from portTypes"
      )
    ...
    if (bindings) {
      ...
      bindingList.forEach((binding: any) => {
        ...
        const portType = portTypeMap.get(bindingPortTypeName)
        ...
        if (!finalPortType && bindingPortTypeName) {
          ...
          if (!finalPortType) {
            console.log(
              "Port type not found, attempting to extract operations directly from binding"
            )
          }
        }
          ...
          ops.forEach((op: any) => {
            ...
            if (!soapAction) {
              ...
              if (soapOperation && soapOperation["@_soapAction"]) {
                ...
                console.log(
                  `Found soap action in soap:operation: ${soapAction}`
                )
              }
            }
            ...
            if (!soapAction) {
              ...
              if (soap12Operation && soap12Operation["@_soapAction"]) {
                ...
                console.log(
                  `Found soap action in soap12:operation: ${soapAction}`
                )
              }
            }
            ...
            if (!soapAction) {
              for (const key in op) {
                if (
                  ...
                  soapAction = op[key]["@_soapAction"]
                  console.log(
                    `Found soap action in child object: ${soapAction}`
                  )
                  break
                }
              }
            ...
            if (finalPortType) {
              ...
              if (portTypeOps) {
                const portTypeOpsArray = Array.isArray(portTypeOps)
                  ? portTypeOps
                  : [portTypeOps]
                console.log(
                  `Looking for matching operation '${opName}' among ${portTypeOpsArray.length} port type operations`
                )

                // Log all port type operation names for debugging
                portTypeOpsArray.forEach((pto) => {
                  console.log(
                    `Available port type operation: ${pto && pto["@_name"]}`
                  )
                })
                ...
                if (!portTypeOp) {
                  portTypeOp = portTypeOpsArray.find((pto) => {
                    const ptoName = pto && pto["@_name"]
                    return (
                      ptoName && ptoName.toLowerCase() === opName.toLowerCase()
                    )
                  })
                }
                ...
                console.log(
                  "Matching port type operation found:",
                  portTypeOp ? "Yes" : "No"
                )
                ...
                if (portTypeOp) {
                  ...
                  const inputMessageName = input?.["@_message"]
                    ?.split(":")
                    .pop()
                  const outputMessageName = output?.["@_message"]
                    ?.split(":")
                    .pop()
    ...
    if (wsdlServices) {
      const servicesArray = Array.isArray(wsdlServices)
        ? wsdlServices
        : [wsdlServices]
      servicesArray.forEach((service: any) => {
        const servicePorts = findElement(service, "port")
        if (servicePorts) {
          const ports = Array.isArray(servicePorts)
            ? servicePorts
            : [servicePorts]
...
export const generateRequestFromWSDL = (
  ...
  const generateFields = (
    ...
    return fields
      .map(
        (field) =>
          `${indent}<${field.name.split(":").pop()}>?</${field.name.split(":").pop()}>`
      )
      .join("\n")
  }
