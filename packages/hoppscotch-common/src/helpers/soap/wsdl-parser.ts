...
export const parseWSDL = async (
  ...
  try {
    ...
    const schemas = new Map<string, WSDLSchema>()    // --- Pre-process portTypes into a map for easy lookup ---
    ...
    console.log("4a. Pre-processed PortTypes:", Object.fromEntries(portTypeMap))// --- Build message map ---
    ...
    console.log("4b. Built Message Map:", Object.fromEntries(messageMap))    // --- Extract schema types ---
    ...
    console.log("5. Extracted Schemas:", Object.fromEntries(schemas))// --- Extract Operations from Bindings (more reliable) ---
      ...
      bindingList.forEach((binding: any) => {
        ...
        if (bindingOps) {
          ...
          ops.forEach((op: any) => {
            ...
            let opDetails: WSDLOperation = existingOp || { name: opName }            // Find soapAction in binding
            const soapOp = findElement(op, 'operation')
            
            // Look for soap:operation or any element ending with :operation for SOAP action
            let soapAction = null
            for (const key in op) {
            ...
            // Try to extract the soapAction from different places
            if (soapOp && soapOp["@_soapAction"]) {
              opDetails.soapAction = soapOp["@_soapAction"]
              console.log(`Found soap action in operation: ${opDetails.soapAction}`)
            } else if (soapAction) {
              opDetails.soapAction = soapAction
            } else {
              // Try a more aggressive search for soapAction
              for (const key in op) {
                if (typeof op[key] === 'object' && op[key] !== null) {
                  // Check for @_soapAction in any child object
                  if (op[key]['@_soapAction']) {
                    opDetails.soapAction = op[key]['@_soapAction']
                    console.log(`Found soap action in child object: ${opDetails.soapAction}`)
                    break
                  }
                }
              }
