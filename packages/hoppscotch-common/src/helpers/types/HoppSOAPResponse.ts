...
): response is HoppSOAPResponse & { type: "success" } {
 ...
 */
export function isSOAPResponseFailure(
  response: HoppSOAPResponse
): response is HoppSOAPResponse & {
  type: "fail" | "network_fail" | "script_fail" | "extension_error"
} {
  return ["fail", "network_fail", "script_fail", "extension_error"].includes(
    response.type
  )
}
 ...
 */
export function isSOAPResponseLoading(
  response: HoppSOAPResponse
): response is HoppSOAPResponse & { type: "loading" } {
