<template>
  <div class="soap-page flex flex-col flex-1 bg-primary">
    <div
      class="flex items-center justify-between p-4 border-b border-dividerLight"
    >
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="w-72 border-r border-dividerLight flex flex-col">
        <div class="flex border-b border-dividerLight">
          <button
            ...
            :class="{
              'bg-primaryLight border-b-2 border-accent':
        ...
        <div class="flex-1 overflow-y-auto p-2">
                ...
                >
                  <div class="flex items-center flex-1 truncate">
                    ...
                    >
                      <path
                        :d="
                          project.expanded ? 'M19 9l-7 7-7-7' : 'M9 5l7 7-7 7'
                        "
                  ...
                  <div class="flex items-center">
                    <span class="text-xs text-secondaryLight mr-2"
                      >{{ project.operations.length }} ops</span
                    >
                    <button
                ...
                <div v-if="project.expanded" class="pl-4 mt-1">
                  ...
                  >
                    ...
                    <div v-if="isServiceExpanded(service.name)" class="pl-6">
                      ...
                      <div
                        v-if="service.port?.address"
                        class="flex items-center p-1.5 text-xs"
                      >
                        <svg
                          class="w-3.5 h-3.5 mr-1.5 text-secondaryLight"
                          viewBox="0 0 24 24"
                          fill="none"
                          stroke="currentColor"
                          stroke-width="2"
                      ...
                      <!-- Service Operations -->
                      <div
                        v-for="op in getOperationsForService(project, service)"
                        :key="op.name"
                      ...
                      >
                        <div class="flex items-center flex-1 min-w-0">
                          <svg
                            class="flex-shrink-0 w-3.5 h-3.5 mr-1.5 text-accent"
                            viewBox="0 0 24 24"
                            fill="none"
                            stroke="currentColor"
                            stroke-width="2"
                  ...
                  <div
                    v-if="getOperationsWithoutService(project).length > 0"
                    class="mt-2"
                  >
                    <div class="text-xs font-medium text-secondaryLight p-1">
                      Other Operations
                    </div>
                    <div
                      v-for="op in getOperationsWithoutService(project)"
                      :key="op.name"
          ...
          <div v-if="sidebarTab === 'history'">
            <p class="text-secondaryLight p-4 text-center">
      ...
      <div class="flex flex-col flex-1 overflow-hidden">
        <div
          ...
          class="flex flex-col items-center justify-center h-full text-secondaryLight"
        >
          <img
            src="/logo.svg"
            alt="Hoppscotch"
            class="w-24 h-24 mb-4 opacity-25"
          />
        ...
        <div v-else class="flex flex-col flex-1 overflow-auto p-4 space-y-4">
          ...
          <div v-for="tab in tabsList" :key="tab.id" class="mb-4">
            <ImprovedSoapUI
              ...
              :initial-expanded="tabExpanded[tab.id] !== false"
    ...
    >
      <div class="bg-primary w-full max-w-md p-6 rounded-lg shadow-lg">
        ...
        <div class="space-y-4">
          <div>
            <label for="projectName" class="block mb-2 font-semibold"
              >Project Name</label
            >
            <input
              id="projectName"
          ...
          <div v-if="wsdlSource === 'url'">
            <label for="wsdlUrlInput" class="block mb-2 font-semibold"
              >WSDL URL</label
            >
            <input
              id="wsdlUrlInput"
          ...
          <div v-if="wsdlSource === 'file'">
            <label for="wsdlFileInput" class="block mb-2 font-semibold"
              >WSDL File</label
            >
            <div
              class="border-2 border-dashed border-dividerLight rounded-lg p-4 text-center"
            >
              <input
                id="wsdlFileInput"
                ref="fileInput"
              ...
              >
                <svg
                  viewBox="0 0 24 24"
                  class="w-8 h-8 mx-auto mb-2 text-secondaryLight"
                >
                  <path
                    fill="currentColor"
            ...
            <div class="space-y-2">
              <div class="flex items-center">
                ...
                <label for="createSampleRequests"
                  >Create sample requests for all operations</label
                >
              </div>
              ...
              <div class="flex items-center">
                <input
                  ...
                  class="mr-2 text-accent rounded focus:ring-accent"
                />
                <label for="createTestSuite"
                  >Create a TestSuite for the imported WSDL</label
                >
              </div>
              ...
              <div class="flex items-center">
                <input
                  ...
                  class="mr-2 text-accent rounded focus:ring-accent"
                />
                <label for="createSimulation"
                  >Create a web service simulation of the imported WSDL</label
                >
              </div>
            </div>
          </div>
    ...
    >
      <div
        class="bg-primary w-full max-w-4xl max-h-[90vh] p-6 rounded-lg shadow-lg overflow-y-auto"
      >
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-xl font-bold">Request Details</h2>
          <button
            class="text-secondary hover:text-accent"
            @click="showRequestDetails = false"
          >
            <svg viewBox="0 0 24 24" class="w-6 h-6">
              <path
                fill="currentColor"
        ...
        <div v-if="selectedRequest" class="space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div
                class="p-3 bg-primaryLight border border-dividerLight rounded"
              >
            ...
            <div class="md:col-span-2">
              ...
              <div
                class="p-3 bg-primaryLight border border-dividerLight rounded break-all"
              >
          ...
          <div
            v-if="selectedRequest.headers && selectedRequest.headers.length > 0"
          >
            <label class="block mb-2 font-semibold">Headers</label>
            <div class="p-3 bg-primaryLight border border-dividerLight rounded">
              <div
                v-for="header in selectedRequest.headers"
                :key="header.key"
                class="mb-1"
              >
          ...
          <div
            v-if="selectedRequest.params && selectedRequest.params.length > 0"
          >
            <label class="block mb-2 font-semibold">Parameters</label>
            <div class="p-3 bg-primaryLight border border-dividerLight rounded">
              <div
                v-for="param in selectedRequest.params"
                :key="param.key"
                class="mb-1"
              >
...
const ImprovedSoapUI = defineAsyncComponent(
  () => import("../components/soap/ImprovedSoapUI.vue")
)
...
const openOperationAsTabFromProject = (
  project: SOAPProject,
  operation: EnhancedWSDLOperation
) => {
  const request = makeSOAPRequest({
    ...
    soapVersion:
      operation.soapVersion === "1.2" ? SOAP_VERSION_1_2 : SOAP_VERSION_1_1,
  })
  ...
  localStorage.setItem(
    "hoppscotch-soap-tab-expanded",
    JSON.stringify(tabExpanded.value)
  )
}
...
const formatResponseXML = (tabId: string) => {
  ...
  try {
    ...
    const responseText =
      typeof tabDoc.response.body === "string"
...
const removeTabOperation = (tab: HoppTab<HoppSOAPTabDocument>) => {
  ...
  const project = soapProjects.value.find(
    (p) =>
      p.wsdlUrl === wsdlUrl &&
...
const sendRequest = async (tabId: string) => {
  ...
  try {
    ...
    if (request.soapVersion !== "1.2") {
      ...
      const headers = request.headers
        ? request.headers.filter(
            (h) => h && h.key && h.key.toLowerCase() !== "soapaction"
          )
        : []
      ...
      if (request.endpoint.includes("dneonline.com")) {
        ...
        console.log(
          "Using special namespace format for Calculator service:",
          soapAction
        )
    ...
    if (
      !modifiedRequest.endpoint ||
      !modifiedRequest.endpoint.startsWith("http")
    ) {
      toast.error(
        "Invalid endpoint URL. Please make sure it starts with http:// or https://"
      )
      return
    }
    ...
    if (
      !modifiedRequest.endpoint.includes("localhost") &&
      !modifiedRequest.endpoint.startsWith(window.location.origin)
    ) {
      console.log(
        "External endpoint detected. Will proceed without proxy unless CORS errors occur."
      )

      // Check if the endpoint already uses a proxy
      const isUsingProxy =
        modifiedRequest.endpoint.includes("corsproxy.io") ||
        modifiedRequest.endpoint.includes("allorigins.win") ||
        modifiedRequest.endpoint.includes("cors-anywhere")

      if (isUsingProxy) {
        console.log(
          "Endpoint is already using a CORS proxy:",
          modifiedRequest.endpoint
        )
      }
    }
    ...
    modifiedRequest.headers.forEach((h) => {
      if (h.key.toLowerCase() === "content-type") {
        ...
        if (
          modifiedRequest.soapVersion === "1.2" &&
          !h.value.includes("soap+xml")
        ) {
          h.value = "application/soap+xml;charset=UTF-8"
        } else if (
          modifiedRequest.soapVersion !== "1.2" &&
          !h.value.includes("xml")
        ) {
    ...
    if (modifiedRequest.body) {
      if (
        !modifiedRequest.body.includes("<Envelope") &&
    ...
    const timeoutId = setTimeout(() => {
      console.warn(
        "SOAP request potentially stuck - no response after 30 seconds"
      )
      toast.error(
        "Request may be stuck. Check network tab for CORS issues or try using a proxy."
      )
    }, 30000)
        ...
        if (response.type !== "loading") {
          ...
          if (response.type === "network_fail") {
            ...
            if (isCORSError) {
              ...
              toast.error(
                `CORS error: The server doesn't allow browser access. You can try enabling the CORS proxy.`
              )
    ...
    const tabDoc = tabService.getTabDocument(tabId)
    const requestInfo =
      tabDoc && tabDoc.type === "request" ? tabDoc.request : null
...
const generateSampleRequestBody = (
  operation: WSDLOperation,
  soapVersion: string = "1.1",
  ...
  // Detect Calculator-like operations dynamically rather than hardcoding
    ...
    const isCalculatorLike =
      ...
      (targetNamespace === "http://tempuri.org/" ||
        targetNamespace.includes("dneonline.com")) &&
  ...
  // For other operation types, try to dynamically determine parameter structure
    ...
    if (inputElement && inputElement.includes("Request")) {
      ...
      if (
        baseOp.startsWith("Get") ||
        baseOp.startsWith("Find") ||
        baseOp.startsWith("Search")
      ) {
...
const previewParseWSDLFile = async (file: File) => {
  try {
    ...
    toast.error(
      `WSDL preview parse error: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
    ...
    if (wsdlSource.value === "url" && wsdlUrl.value) {
      try {
        ...
        newProject.operations = (wsdlData.operations || []).map((op) => {
          ...
          const serviceForOperation = findServiceForOperation(
            wsdlData.services,
            op
          )

          // Find the service object for the resolved service name
          const serviceObj = wsdlData.services.find(
            (s) => s.name === serviceForOperation
          )

          // Get the endpoint from the service if available, otherwise use selected endpoint
          const endpointForOperation =
            serviceObj?.port?.address || selectedEndpoint.value || wsdlUrl.value
          ...
          if (importOptions.value.createSampleRequests) {
            ...
            enhancedOp.sampleBody = generateSampleRequestBody(
              op,
              "1.1",
              wsdlData.targetNamespace || "http://tempuri.org/"
            )
          }
        ...
        toast.success(
          `WSDL parsed successfully. Found ${newProject.operations.length} operations.`
        )
      } catch (error) {
        toast.error(
          `WSDL parsing error: ${error instanceof Error ? error.message : String(error)}`
        )
      ...
      try {
        ...
        newProject.operations = (wsdlData.operations || []).map((op) => {
          ...
          // Get the endpoint from the service if available, otherwise use selected endpoint
          const endpointForOperation =
            serviceObj?.port?.address || selectedEndpoint.value || ""
...
const removeProject = (projectId: string) => {
  ...
  if (index !== -1) {
    ...
    confirmDialogAction.value = () => {
      ...
      const projectOperationNames = soapProjects.value[index].operations.map(
        (op) => op.name
      )
      ...
      tabsList.value.forEach((tab) => {
        ...
        if (tabDoc && tabDoc.type === "request" && tabDoc.request) {
          ...
          const wsdlMatches =
            projectWsdlUrl && tabDoc.request.wsdlUrl === projectWsdlUrl

          // Match by operation name (secondary method)
          const operationMatches =
            tabDoc.request.operation &&
          ...
          const endpointMatches =
            soapProjects.value[index].endpoint &&
      ...
      // Log for debugging
      console.log(
        `Found ${tabsToClose.length} tabs to close for project "${projectName}"`
      )
      ...
      if (tabsToClose.length > 0) {
        toast.success(
          `Project "${projectName}" removed and ${tabsToClose.length} related tab(s) closed`
        )
      } else {
...
const removeOperation = (projectId: string, operationName: string) => {
  ...
  if (projectIndex !== -1) {
    ...
    const operationIndex = project.operations.findIndex(
      (op) => op.name === operationName
    )

    if (operationIndex !== -1) {
      // Count how many tabs will be closed
      const tabsWithOperation = tabsList.value.filter(
        (tab) =>
      ...
      try {
        ...
        if (
          ...
          (service.port?.binding &&
            operation.soapAction.includes(
              service.port.binding.split(":").pop() || ""
            ))
        ) {
          return service.name
        }
  ...
  for (const service of services) {
    if (service.port?.binding) {
      ...
      if (
        operation.name &&
        bindingName &&
        (bindingName.startsWith(service.name) ||
          service.name.includes(
            bindingName.replace("Soap", "").replace("SOAP", "")
          ))
      ) {
        return service.name
      }
...
onMounted(() => {
  try {
    ...
    const savedExpandedCards = localStorage.getItem(
      "hoppscotch-soap-expanded-cards"
    )
    ...
    const savedExpandedServices = localStorage.getItem(
      "hoppscotch-soap-expanded-services"
    )
